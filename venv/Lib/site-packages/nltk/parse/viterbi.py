...
class ViterbiParser(ParserI):
    """
    A bottom-up ``PCFG`` html_parser that uses dynamic programming to find
    the single most likely parse for a text.  The ``ViterbiParser`` html_parser
    ...
    The ``ViterbiParser`` html_parser fills in this table incrementally.  It starts
    ...
    node value, the ``ViterbiParser`` html_parser considers all productions that
    ...
    def __init__(self, grammar, trace=0):
        """
        Create a new ``ViterbiParser`` html_parser, that uses ``grammar`` to
        parse texts.
