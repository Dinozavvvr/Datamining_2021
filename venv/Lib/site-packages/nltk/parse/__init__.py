...
However, the html_parser module does *not* distinguish these two types of
ambiguity.

The html_parser module defines ``ParserI``, a standard interface for parsing
...
three sub-modules for specialized kinds of parsing:
  ...
  - ``nltk.html_parser.chart`` defines chart parsing, which uses dynamic
    programming to efficiently parse texts.
  - ``nltk.html_parser.probabilistic`` defines probabilistic parsing, which
