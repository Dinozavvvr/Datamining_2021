...
"""
Data classes and html_parser implementations for "chart parsers", which
use dynamic programming to efficiently parse a text.  A chart
html_parser derives parse trees for a text by iteratively adding "edges"
to a "chart."  Each edge represents a hypothesis about the tree
structure for a subsequence of the text.  The chart is a
"blackboard" for composing and combining these hypotheses.

When a chart html_parser begins parsing a text, it creates a new (empty)
...
the ``TreeEdge`` and ``LeafEdge`` classes.  The chart html_parser module
defines three chart parsers:

  - ``ChartParser`` is a simple and flexible chart html_parser.  Given a
...
class ChartParser(ParserI):
    """
    A generic chart html_parser.  A "strategy", or list of
    ...
    def __init__(
        ...
        """
        Create a new chart html_parser, that uses ``grammar`` to parse
        texts.
...
class SteppingChartParser(ChartParser):
    ...
    change the html_parser's strategy or grammar midway through parsing a
    text.

    The ``initialize`` method is used to start parsing a text.  ``step``
    adds a single edge to the chart.  ``set_strategy`` changes the
    strategy used by the chart html_parser.  ``parses`` returns the set of
    parses that has been found by the chart html_parser.

    :ivar _restart: Records whether the html_parser's strategy, grammar,
    ...
    def step(self):
        ...
        If the html_parser's strategy, grammar, or chart is changed, then
    ...
    def _parse(self):
        ...
        whenever the html_parser's strategy, grammar, or chart is modified.
        """
        chart = self._chart
        grammar = self._grammar
        edges_added = 1
    ...
    def strategy(self):
        "Return the strategy used by this html_parser."
        return self._strategy

    def grammar(self):
        "Return the grammar used by this html_parser."
        return self._grammar

    def chart(self):
        "Return the chart that is used by this html_parser."
        return self._chart
    ...
    def set_strategy(self, strategy):
        """
        Change the strategy that the html_parser uses to decide which edges
        to add to the chart.
    ...
    def set_grammar(self, grammar):
        "Change the grammar used by the html_parser."
    ...
    def set_chart(self, chart):
        "Load a given chart into the chart html_parser."
...
def demo(
    ...
    # Ask the user which html_parser to test,
    # if the html_parser wasn't provided as an argument
    if choice is None:
        print("  1: Top-down chart html_parser")
        print("  2: Bottom-up chart html_parser")
        print("  3: Bottom-up left-corner chart html_parser")
        print("  4: Left-corner chart html_parser with bottom-up filter")
        print("  5: Stepping chart html_parser (alternating top-down & bottom-up)")
        print("  6: All parsers")
        print("\nWhich html_parser (1-6)? ", end=" ")
        choice = sys.stdin.readline().strip()
        print()

    choice = str(choice)
    if choice not in "123456":
        print("Bad html_parser number")
        return

    # Keep track of how long each html_parser takes.
    times = {}
    ...
    # Run the requested chart html_parser(s), except the stepping html_parser.
    ...
    # Run the stepping html_parser, if requested.
    if choice in "56":
    ...
    format = "%" + repr(maxlen) + "s html_parser: %6.3fsec"
