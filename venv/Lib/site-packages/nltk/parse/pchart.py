...
probabilistic html_parser module defines ``BottomUpProbabilisticChartParser``.

``BottomUpProbabilisticChartParser`` is an abstract class that implements
a bottom-up chart html_parser for ``PCFG`` grammars.  It maintains a queue of edges,
...
html_parser to expand more likely edges before less likely ones.  Each
...
class BottomUpProbabilisticChartParser(ParserI):
    """
    An abstract bottom-up html_parser for ``PCFG`` grammars that uses a ``Chart`` to
    ...
    def __init__(self, grammar, beam_size=0, trace=0):
        ...
        :type beam_size: int
        :param beam_size: The maximum length for the html_parser's edge queue.
        :type trace: int
...
class InsideChartParser(BottomUpProbabilisticChartParser):
    """
    A bottom-up html_parser for ``PCFG`` grammars that tries edges in descending
...
# Eventually, this will become some sort of inside-outside html_parser:
...
class RandomChartParser(BottomUpProbabilisticChartParser):
    """
    A bottom-up html_parser for ``PCFG`` grammars that tries edges in random order.
...
class UnsortedChartParser(BottomUpProbabilisticChartParser):
    """
    A bottom-up html_parser for ``PCFG`` grammars that tries edges in whatever order.
    """
...
class LongestChartParser(BottomUpProbabilisticChartParser):
    """
    A bottom-up html_parser for ``PCFG`` grammars that tries longer edges before
...
def demo(choice=None, draw_parses=None, print_parses=None):
    ...
    be found; and then each html_parser is run on the same demo, and a
