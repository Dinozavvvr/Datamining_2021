...
class RecursiveDescentParser(ParserI):
    """
    A simple top-down CFG html_parser that parses texts by recursively
    ...
    When the html_parser begins parsing a text, it constructs a tree
    ...
    def _trace_tree(self, tree, frontier, operation):
        """
        Print trace output displaying the html_parser's current state.
...
class SteppingRecursiveDescentParser(RecursiveDescentParser):
    ...
    of parses that have been found by the html_parser.

    :ivar _history: A list of ``(rtext, tree, frontier)`` tripples,
        containing the previous states of the html_parser.  This history is
    ...
    def initialize(self, tokens):
        """
        Start parsing a given text.  This sets the html_parser's tree to
    ...
    def backtrack(self):
        """
        Return the html_parser to its state before the most recent
        match or expand operation.  Calling ``undo`` repeatedly return
        the html_parser to successively earlier states.  If no match or
    ...
    def expandable_productions(self):
        ...
        :return: A list of all the productions for which expansions
            are available for the current html_parser state.
    ...
    def untried_expandable_productions(self):
        ...
        :return: A list of all the untried productions for which
            expansions are available for the current html_parser state.
        :rtype: list(Production)
        """
    ...
    def currently_complete(self):
        """
        :return: Whether the html_parser's current state represents a
            complete parse.
        :rtype: bool
        """
    ...
    def _parse(self, remaining_text, tree, frontier):
        ...
        capture these recursive calls.  It records the html_parser's old
        state in the history (to allow for backtracking), and updates
        the html_parser's new state using the given arguments.  Finally, it
...
def demo():
    """
    A demonstration of the recursive descent html_parser.
    """
