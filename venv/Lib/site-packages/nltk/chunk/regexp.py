...
class RegexpChunkParser(ChunkParserI):
    """
    A regular expression based chunk html_parser.  ``RegexpChunkParser`` uses a
...
class RegexpParser(ChunkParserI):
    """
    A grammar based chunk html_parser.  ``chunk.RegexpParser`` uses a set of
    regular expression patterns to specify the behavior of the html_parser.
    ...
    chunk html_parser is one having more than one clause.  The maximum depth
    of a parse tree created by this chunk html_parser is the same as the
    ...
    def __init__(self, grammar, root_label="S", loop=1, trace=0):
        """
        Create a new chunk html_parser, from the given start state
    ...
    def _add_stage(self, rules, lhs, root_label, trace):
        """
        Helper function for __init__: add a new stage to the html_parser.
        """
        if rules != []:
            if not lhs:
    ...
    def parse(self, chunk_struct, trace=None):
        """
        Apply the chunk html_parser to this input.
...
def demo_eval(chunkparser, text):
    """
    Demonstration code for evaluating a chunk html_parser, using a
    ``ChunkScore``.  This function assumes that ``text`` contains one
    sentence per line, and that each sentence has the form expected by
    ``tree.chunk``.  It runs the given chunk html_parser on each sentence in
    ...
    # Evaluate our chunk html_parser.
