...
In order to construct a html_parser, you also need a rule set.
The standard English rules are provided in chart as
``chart.DefaultRuleSet``.

The html_parser can then be constructed by calling, for example:
``html_parser = chart.CCGChartParser(<lexicon>, <ruleset>)``

Parsing is then performed by running
``html_parser.parse(<sentence>.split())``.
