...
"""
A graphical tool for exploring the shift-reduce html_parser.

The shift-reduce html_parser maintains a stack, which records the structure
...
The html_parser builds up a tree structure for the text using two
operations:
...
You can control the html_parser's operation by using the "shift" and
"reduce" buttons; or you can use the "step" button to let the html_parser
automatically decide which operation to apply.  The html_parser uses the
...
class ShiftReduceApp(object):
    """
    A graphical tool for exploring the shift-reduce html_parser.  The tool
    displays the html_parser's stack and the remaining text, and allows the
...
def app():
    """
    Create a shift reduce html_parser app, using a simple grammar and
    text.
    """
