...
"""
A graphical tool for exploring the recursive descent html_parser.

The recursive descent html_parser maintains a tree, which records the
...
The html_parser builds up a tree structure for the text using three
operations:
...
The html_parser maintains a list of tree locations called a "frontier" to
...
html_parser always performs expand and match operations on the leftmost
element of the frontier.

You can control the html_parser's operation by using the "expand," "match,"
and "backtrack" buttons; or you can use the "step" button to let the
html_parser automatically decide which operation to apply.  The html_parser uses
...
The "autostep" button will let the html_parser continue applying
...
class RecursiveDescentApp(object):
    """
    A graphical tool for exploring the recursive descent html_parser.  The tool
    displays the html_parser's tree and the remaining text, and allows the
    user to control the html_parser's operation.  In particular, the user
...
def app():
    """
    Create a recursive descent html_parser demo, using a simple grammar and
    text.
    """
